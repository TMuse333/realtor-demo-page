...
const {textColor, darkMode} = useGeneralContext()

    return (
        <>
        {title && description && (
            <>
           <h2 className={`bg-gradient-to-b ${darkMode ? 'from-white to-gray-300 bg-clip-text text-transparent' : 'text-black' }
           ...
           <p className={`w-[80vw] mx-auto text-xl
                transition-opacity
               text-lg sm:text-xl my-8 text-${textColor}`}>
            {description}</p>
           </>
        )}
