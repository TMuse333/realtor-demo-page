...
// 4 by 6 feet, 2 by 3 inches
  const bigLinks = [
   ...
   
    {
      ...
      subMenuSrc:trophy.src,
      subMenuAlt:'Two hands',
      desktopDescription:'Here are the pages where you can showcase buying or selling a home'
    },
    {
      name: 'Contact',
  ...
  const DesktopSubMenu:React.FC<DesktopSubMenuProps> = ({
    ...
    return (
        <div className={` fixed top-[25px] w-screen
         left-0  transition-all overflow-hidden
         pt-4  text-white
        flex justify-evenly ${isClicked  ? 'h-[200px] bg-gray-800 z-[10] opacity-1' : 'h-[0px] z-[-1] bg-[transparent] opacity-0'}`
        }
...
<ul className="flex mt-6">
  ...
  {links.map((link, index) => (
    <li
      key={index}
      className={`text-md sm:text-xl mb-4 mr-auto text-white pl-2 pr-2 hover:text-gray-200]`}>
      {link.destination ? (
        <Link href={link.destination}>
          {link.name}
        </Link>
          ...
          {link.name}
                    ...
                    ${!secondarySubMenuClicked ? 'translate-x-full': ''}`}>
                            ...
                            <button onClick={handleSecondarySubLeave}
                             ...
                             bg-transparent !text-lg hover:text-gray-200
...
const {heroInView} = useGeneralContext()
    ...
    return (
        <nav className={`fixed top-[0%] z-[301] w-screen left-0  h-[30px] flex justify-between items-center
          ${heroInView ? 'bg-gray-300' : 'bg-gray-300'}`}>
            <MobileSubMenu
          ...
          mx-auto 
            ...
            {filteredLinks.map((link,index) => (
                ...
                <div key={index}
                 ...
                 my-auto ${!heroInView ? 'text-black hover:text-white' : 'text-black'}`}
